...
tibble(gamma = seq(0, 0.9, 0.1)) %>%
  mutate(lookahead = translate_gamma_to_lookahead(gamma)) %>%
  kable()
```
...
tibble(lookahead = 1:10) %>%
  mutate(gamma = translate_lookahead_to_gamma(lookahead)) %>%
  kable()
```

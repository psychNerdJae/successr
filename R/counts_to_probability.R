#' @title Convert successor counts into probabilities
#' @name counts_to_probability
#'
#' @description
#' Normalizes the successor matrix. Technically, the successor values are
...
#' @export
#'
counts_to_probability <- function(successor_values,
                                  value_col_name = NULL,
                                  gamma_value = NULL) {

  if (is.data.frame(successor_values)) {
    output <- successor_values %>%
      mutate({{value_col_name}} := {{value_col_name}} * (1 - gamma))
  } else if (is.matrix(successor_values)) {
    output <- successor_values * (1 - gamma_value)
  } else {
